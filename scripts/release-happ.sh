...
#   1. `workdir/glassbeadgame.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/glassbeadgame.happ"
else
  happ="workdir/glassbeadgame.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical glassbeadgame.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download glassbeadgame.happ from $HAPP_TAG."
