# Releasing Glass Bead Game

Glass Bead Game is distributed as a `.webhapp` referenced by the
...
and 0.6 and 0.7 agents form disjoint networks. Games created on the 0.6 line
(Glass Bead Game 0.3.x) do not carry over, and a group has to move across
together — a 0.7 agent cannot see 0.6 peers at all.

Glass Bead Game has **no export/import feature**, so there is no way to carry a
game's beads or comments across. Anyone still wanting the 0.6 content has to keep
a 0.6 Moss installed alongside. Say this plainly in the curation changelog.
...
over every zome named in `dnas/glassbeadgame/workdir/dna.yaml` (`wasm-opt -Oz
...
The script verifies `workdir/glassbeadgame.happ` against `.happ-sha256` and
...
3. The [`release-webhapp`](.github/workflows/release-webhapp.yaml) workflow then:
   - downloads the frozen happ from `happ-v<dnaVersion>` and checks its sha256
     against `.happ-sha256`,
   - builds the UI and packs `glassbeadgame.webhapp` (no `--recursive`, so the happ
   ...
   - publishes a **prerelease** GitHub release with `glassbeadgame.webhapp` attached.
...
Add a new `versions[]` entry for the `glass-bead-game` tool id in the 0.16 curation list with the
